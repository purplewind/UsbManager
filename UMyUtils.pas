...
type
  ...
  public
    ...
    class function getIsUsb( DriverPath : string ): Boolean;
    class function getDriverID( DriverPath : string ): string;
    class function getExistUsb : Boolean;
    class function getUseList : TStringList;
  public
...
class function MyFilePath.getDriverID(DriverPath: string): string;
var
  NotUsed: DWORD;
  VolumeFlags: DWORD;
...
begin
try
  GetVolumeInformation( Pchar( DriverPath ), @VName, SizeOf(VolumeInfo), @VSNumber,
                        NotUsed, VolumeFlags, PType, 32);
  Result := InttoHex(VSNumber,8);
except
  Randomize;
...
class function MyFilePath.getExistUsb: Boolean;
var
...
begin
  ...
  begin
    DriverPath := StrArray[i];
    if getDriverExist( DriverPath ) and getIsUsb( DriverPath ) then
    begin
      Result := True;
      Exit;
    end;
...
class function MyFilePath.getIsUsb(DriverPath: string): Boolean;
begin
  try
    DriverPath := getPath( DriverPath );
    Result := GetDriveType(Pchar(DriverPath)) = DRIVE_Removable;
  except
    Result := False;
  end;
end;
...
class function MyFilePath.getUseList: TStringList;
var
  StrArray : TStringDynArray;
  i: Integer;
  DriverPath : string;
